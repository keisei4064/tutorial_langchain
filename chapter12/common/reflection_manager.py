...
class ReflectionManager:
    ...
    # リフレクション内容を更新&ファイルに保存
    def save_reflection(self, reflection: Reflection) -> str:
        reflection.id = str(uuid.uuid4())
        reflection_id = reflection.id
        self.reflections[reflection_id] = reflection  # リフレクション内容を保存
        embedding = self.embeddings.embed_query(reflection.reflection)
        self.embeddings_dict[reflection_id] = embedding  # 反省文の埋め込みを保存

        if self.index is None:
            # インデックス生成
            #   L2距離（ユークリッド距離）を使って線形探索（brute-force）で近いベクトルを見つける
            self.index = faiss.IndexFlatL2(len(embedding))
        self.index.add(
            np.array([embedding]).astype("float32")
        )  # embeddingをFAISSの検索対象として登録する
    ...
    def get_relevant_reflections(self, query: str, k: int = 3) -> list[Reflection]:
        ...
        try:
            D, I = self.index.search(  # 近いベクトル(のインデックス)を探す
                np.array([query_embedding]).astype("float32"),  # 埋め込みベクトル
                min(k, len(self.reflections)),  # 取得するリフレクション数
            )
            # I: 近いベクトル(のインデックス)

            # インデックス番号を実際のデータに対応させる

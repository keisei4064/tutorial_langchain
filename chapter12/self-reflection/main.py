...
class ReflectiveAgentState(BaseModel):
    ...
    )  # リフレクション内容を永続化するデータベースへのID
    final_output: str = Field(default="", description="最終的な出力結果")
    retry_count: int = Field(default=0, description="タスクの再試行回数")  # 無限ループ回避用
...
class ReflectiveAgent:
    ...
    def _create_graph(self) -> StateGraph:
        ...
        graph.add_conditional_edges(
            "reflect_on_task",
            self._should_retry_or_continue,  # 分岐
            {
                "retry": "execute_task",
    ...
    def _should_retry_or_continue(self, state: ReflectiveAgentState) -> str:
        ...
        if (
            ...
            # リフレクション時にリトライが必要と判断された場合
            return "retry"
        elif state.current_task_index < len(state.tasks) - 1:
            # やるべきタスクが残っている場合
            return "continue"
        else:
            # リトライの必要がなく，また全てのタスクが完了した場合
            return "finish"
...
def main():
    ...
    parser.add_argument("--task", type=str, help="実行するタスク", default="カレーライスの作り方")
